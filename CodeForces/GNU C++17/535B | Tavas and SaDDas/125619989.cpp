...
const int BIG = 1e9;
void push_all_lucky(long long num,vector<int> &all_lucky)
{
    if(num>BIG) return;
    if (num)
        all_lucky.push_back(num);
    push_all_lucky(num*10+4,all_lucky);
    push_all_lucky(num*10+7,all_lucky);
}
int main()
{
    int n;
